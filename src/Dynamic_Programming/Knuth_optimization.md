...
Knuth's Optimization 的應用可說是非常簡單，以[Atcoder DP contest-Slime](https://atcoder.jp/contests/dp/tasks/dp_n)為例(在此就不詳細解釋細節)，我們可以列出這個 DP 式：
...
若我們知道這題可套用 Knuth's Optimization...
...
for(len=2;len<=n;len++)
{
    for(i=1;i+len-1<=n;i++)
    {
        ...
        for(k=optl;k<=optr;k++)
        {
            dp[i][j] = min(dp[i][j], w(i,j) + dp[i][k] + dp[k+1][j]);
            opt[i][j] = k;  
        }
    }
}
...
\\(opt[i][j-1]\\) 為 \\(dp[i][j]\\) 的最佳轉移點，如此一來複雜度就降到 \\(O(n^2)\\) 了！

接下來我們來詳細解釋 Knuth's Optimization 實際上做了什麼。

## 四邊形不等式與轉移點單調

Knuth's Optimization 告訴我們說，當 DP 的代價函數 \\(w\\) 滿足**四邊形不等式**與**區間單調性**時，DP 的轉移來源會有單調性，可以藉此加速 DP。
...
觀察可發現代價函數滿足四邊形不等式，因此可直接套用 Knuth Optimization 解決。
...
若定義 \\(dp(i,j)\\) 代表建一顆BST包含區間 \\([i,j]\\) 元素的最小成本，將其以 \\(k\\) 為根結點，左右子樹為 \\([i, k-1], [k+1,j]\\) 兩顆BST即可遞迴求解， 轉移式：
\\[dp(i,j) = \underset{i\le k\le j}{min}\\{dp(i,k-1) + dp(k+1,j) + w(i,j) - f(k)\\}\\]
\\(w(i,j)\\) 是區間和，這樣就是一個經典的區間 DP，\\(O(n^3)\\)。

而我們知道代價函數 \\( w(i,j) \\) (區間和)會滿足Knuth's Optimization 的條件，因此可優化到 \\(O(n^2)\\)。
</details>
